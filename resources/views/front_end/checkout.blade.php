...
<section class="section">
    ...
    <aside class="left"  id="divaddress">
        ...
        @if(session('cart'))
            @foreach((array) session('cart') as $id => $details)
                ...
                <tr>
                    ...
                    <td data-th="Product"><h4 class="nomargin">{{ $details['name'] }}</h4></td>
        ...
        <tr class="visible-xs">
            ...
            <td  class="text-center" style="text-align:right;font-weight: bold;"><strong> Rs:&nbsp;<span class="cart-total">{{ $total }}</span></strong></td>
        </tr>
        </tfoot>
    </table>
    <h3>Address Details</h3>
        @php
			...
			<div class="clear">
            <input name="addresscheckbox" class="checkbox-custom checkboxaddress" id="{{$row_info->id}}" type="checkbox" value="{{$row_info->id}}">
			<label class="checkbox-custom-label" for="{{$row_info->id}}" >
             <p><strong>{{$row_info->name}}</strong></p>
            <p><strong>Address</strong><br>{{$row_info->address}}</p>
            <p><strong>Mobile No :</strong><br>+91-{{$row_info->mobile}}</p>
            </label></div>           
            @endforeach
        </aside>
        ...
        <fieldset class="right">
			...
			<form action="add_orderdetails" method="post" id="checkout_form">
                ...
                <script src="https://checkout.razorpay.com/v1/checkout.js"
                        ...
                        data-image="{{ env('APP_URL') }}assets/front_end/images/franchise-logo.png"
                        data-prefill.name=""
