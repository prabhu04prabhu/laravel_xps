@extends('front_end.productheader')
@section('content')
<?php
session_start(); 
if(isset($_GET["cid"])){
...
<section class="section">
    <section class="section-content products clear">
        <!-- <aside class="sidebar left">
            @include('front_end.sidebar-products') 
        </aside> -->
...
<fieldset class="clear">
    <form class="form-horizontal m-t-30" action="filter_search" method="POST">
        ...
        <option value="0">All Product Type</option>
                ...
                <option value="{{$row_shop->CategoryID}}" @if ($_SESSION["scategoryid"] == $row_shop->CategoryID) Selected @endif> {{$row_shop->CategoryName}}</option>
                @endforeach
        ...
        <!-- </div> -->
            ...
            <option value="0" >All Vehicle Brand</option>
               <!--  @php
                ...
                @foreach ($vechicle_brand as $row_vbrand)
                <option value="{{$row_vbrand->v_id}}" @if ($_SESSION["sbrandid"]== $row_vbrand->v_id) Selected @endif >{{$row_vbrand->brand_name}}</option>
                @endforeach  -->              
            </select>
            ...
            <option value="0">All Vehicle Model</option>
                ...
                @foreach ($vechicle_model as $row_vmodel)
                <option value="{{$row_vmodel->m_id}}" @if ($_SESSION["smodelid"] == $row_vmodel->m_id) Selected @endif >{{$row_vmodel->model_name}}</option>
                @endforeach   -->             
            </select>
            <?php 
...
function addcart(id){ 
    ...
    //var price=productprice - (productprice * {{env('DEFAULT_DISCOUNT')}}/100);
        $.ajax({
            url: '{{ url('add-to-cart') }}',
            method: "POST",
            data: {quantity: 1,
