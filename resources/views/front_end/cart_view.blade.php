...
<aside class="continue-shop">
    ...
    <aside class="cart-product left" id="cart_products">
        ...
        <article class="clear">
            ...
            <aside>
                <h3>Item</h3>
                <div class="clear">
                    <figure><img src="{{ $details['photo'] }}"></figure>
                    <h4>{{ $details['name'] }}</h4>
                </div>
            </aside>
            <aside>
                ...
                <div class="clear">
                    <div class="quantity-center">
                        {{-- <div class="value-button" id="decrease" onclick="decreaseValues()" value="Decrease Value">-</div>
                        <input type="number" id="number" value="0">
                        <div class="value-button" id="increase" onclick="increaseValues()" value="Increase Value">+</div> --}}

                        <span class="value-button" id="decrease" onclick="decreaseValuesPage({{ $details['productid'] }})" data-id="{{ $id }}" value="Decrease Value">-</span>
                	<input type="number" class="number" id="number{{ $details['productid'] }}" value="{{ $details['quantity'] }}" readonly="true" class="form-control quantity" />
                	<span class="value-button" id="increase" onclick="increaseValuesPage({{ $details['productid'] }})"  data-id="{{ $id }}" value="Increase Value">+</span>
                </div>
            ...
            <aside>
                <h3>Subtotal</h3>
                <div class="clear">
                    <h1>₹ {{ ($details['price'] * $details['quantity']) - $details['scrabamount'] }}</h1>

                    <!-- <button class="btn btn-info btn-sm update-cart" "><i class="fa fa-refresh"></i></button> -->
                    <button class="btn btn-danger btn-sm remove-from-cart" data-id="{{ $id }}" onclick="remove_from_cart({{$id}})"><i class="fa fa-trash-o"></i></button>
    ...
    <aside class="cart-price right">
        <h2>Total</h2>
        <h3>Sub Total : <span id="cart_subtotal">₹{{ $total }}</span></h3>
        <h3>Order Total : <span id="cart_total">₹{{ $total }}</span></h3>
        <!-- <h3 class="hidden-xs text-center"><strong>Total $<span class="cart-total">{{ $total }}</span></strong></h3> -->
...
$("#btncontinue").click(function (e) {
    ...
    else
    {
        ...
        window.location = '{{ url('login-and-register') }}';
    }
        ...
        $(".update-cart").click(function (e) 
        {
            ...
            $.ajax({
                url: '{{ url('update-cart') }}',
                method: "patch",
                data: {_token: '{{ csrf_token() }}', id: ele.attr("data-id"), quantity: $("#number"+ele.attr("data-id")).val()},
                dataType: "json",
    ...
    <script>
        ...
        function increaseValuesPage(id) {
            ...
            $.ajax({
                url: '{{ url('update-cart') }}',
                method: "patch",
                data: {_token: '{{ csrf_token() }}', id: id, quantity: $("#number"+id).val()},
                dataType: "json",
                success: function (response) {
                    $('#cart_products').empty();
                    console.log(response);
